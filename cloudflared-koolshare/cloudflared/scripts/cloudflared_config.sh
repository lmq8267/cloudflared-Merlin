...
fun_nat_start(){
    if [ "${cloudflared_enable}"x = "1"x ] ;then
	    [ ! -L "/koolshare/init.d/S89cloudflared.sh" ] && ln -sf /koolshare/scripts/cloudflared_config.sh /koolshare/init.d/S89cloudflared.sh
    fi
}
# 定时任务
fun_crontab(){
    if [ "${cfd_enable}" != "1" ] || [ "${cfd_cron_time}"x = "0"x ];then
     ...
     if [ "${cfd_cron_hour_min}" == "min" ] && [ "${cfd_cron_time}"x != "0"x ] ; then
        if [ "${cfd_cron_type}" == "watch" ]; then
        	cru a cloudflared_monitor "*/"${cfd_cron_time}" * * * * /bin/sh /koolshare/scripts/cloudflared_config.sh watch"
        elif [ "${webd_cron_type}" == "start" ]; then
            cru a cloudflared_monitor "*/"${cfd_cron_time}" * * * * /bin/sh /koolshare/scripts/cloudflared_config.sh restart"
    	fi
    elif [ "${cfd_cron_hour_min}" == "hour" ] && [ "${cfd_cron_time}"x != "0"x ] ; then
        if [ "${cfd_cron_type}" == "watch" ]; then
            cru a cloudflared_monitor "0 */"${cfd_cron_time}" * * * /bin/sh /koolshare/scripts/cloudflared_config.sh watch"
        elif [ "${cfd_cron_type}" == "start" ]; then
            cru a cloudflared_monitor "0 */"${cfd_cron_time}" * * * /bin/sh /koolshare/scripts/cloudflared_config.sh restart"
        fi
    fi
}
